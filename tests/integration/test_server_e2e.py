...
def test_chat_stream(server_fixture: subprocess.Popen[str]) -> None:
    ...
    data = {
        ...
        "new_message": {
            "role": "user",
            "parts": [{"text": '{"amount": 50.0, "submitter": "alice@company.com", "category": "software", "description": "IDE License", "date": "2026-06-06"}'}],
        },
        "streaming": True,
    }
    ...
    for event in events:
        ...
        if output and output.get("status") == "approved":
            has_approved_status = True
            break

    assert has_approved_status, "Expected at least one event with approved status"
